...
public sealed class CloudDurableTestRunner<TInput, TOutput> : IDurableTestRunner<TInput, TOutput>, IAsyncDisposable
{
    ...
    private readonly bool _ownsLambdaClient;
        ...
        CloudTestRunnerOptions? options = null)
    {
        ...
        // We own (and must dispose) only a client we created ourselves; a
        // caller-supplied client is the caller's to manage.
        _ownsLambdaClient = lambdaClient is null;
        ...
        string executionName, CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            ...
            await Task.Delay(backoff.Next(), cancellationToken);
        }
    }

    // Exponential backoff that starts at InitialPollInterval, doubles each call,
    // and caps at PollInterval. If the caller configured InitialPollInterval to be
    // larger than PollInterval, every delay is just PollInterval.
    private Backoff CreateBackoff() => new(_options.InitialPollInterval, _options.PollInterval);
    ...
    private struct Backoff
    {
        ...
        {
            _max = max;
            _current = initial < max ? initial : max;
        }
        ...
        public TimeSpan Next()
        {
            ...
            _current = doubled < _max ? doubled : _max;
            return delay;
        }
    }
    ...
    // ARNs are returned unchanged. Partition-agnostic: works for the standard
    // "arn:aws:", GovCloud "arn:aws-us-gov:", and China "arn:aws-cn:" partitions
    // since the colon-split layout is identical across all of them.
    private static string StripQualifier(string functionArn)
    {
        if (!functionArn.StartsWith("arn:", StringComparison.Ordinal))
            return functionArn;

        // arn:<partition>:lambda:region:acct:function:name[:qualifier] — the
        // qualifier, if present, is the 8th colon-delimited field (index 7).
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        while (true)
        {
            ...
            await Task.Delay(backoff.Next(), timeoutCts.Token);
        }
    }
    ...
    {
        // Dispose only a client we created. A caller-supplied client is not
        // ours to dispose — the caller may keep using it after the runner.
        if (_ownsLambdaClient)
