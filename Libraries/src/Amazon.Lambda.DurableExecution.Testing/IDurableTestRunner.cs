...
/// against this interface can run unchanged against either backend. Implements
/// <see cref="IAsyncDisposable"/> so callers can <c>await using</c> a runner
/// declared as the interface type.
/// </summary>
public interface IDurableTestRunner<TInput, TOutput> : IAsyncDisposable
{
    /// <summary>
    ...
    /// runner) are resolved automatically. Throws <see cref="InvalidOperationException"/>
    /// if the workflow suspends waiting on a callback — use the two-call pattern
    /// (<see cref="StartAsync"/> + <see cref="WaitForCallbackAsync"/> +
    /// SendCallback* + <see cref="WaitForResultAsync"/>) for callback workflows.
    /// </summary>
