...
internal sealed class ExecutionOrchestrator<TInput, TOutput>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            ...
            // that is a suspend point (StartAsync) or an error (RunAsync).
            if (stopAtSuspend || HasPendingCallback(arn))
                return null;

            // Otherwise the workflow is pending on a real timer or retry backoff
            // (only reachable with SkipTime disabled — time-skipping folds waits/
            // retries to ready before we get here). Delay until the next scheduled
            // resume time before re-invoking; otherwise the loop would spin the CPU,
            // inflate InvocationCount, and hit MaxInvocations long before the timer
            ...
            if (TryGetNextResumeDelay(arn, out var delay) && delay > TimeSpan.Zero)
            {
                ...
                // OperationCanceledException, keeping a single timeout throw-site.
                try
                {
