...
namespace EventsTests.NET8
{
    public class AppSyncTests
    {
        ...
        public void AppSyncTestLambdaAuthorizerResponseEvent(Type serializerType)
        {
            ...
            // Deserialize both the actual and expected JSON for comparison
            var actualObject = JsonSerializer.Deserialize<AppSyncAuthorizerResult>(json);
            var expectedJson = File.ReadAllText("appsync-event-lambda-authorizer-response.json");
            var expectedObject = JsonSerializer.Deserialize<AppSyncAuthorizerResult>(expectedJson);

            // Assert: Compare the deserialized objects
            Assert.Equal(expectedObject.IsAuthorized, actualObject.IsAuthorized);
            Assert.Equal(expectedObject.TtlOverride, actualObject.TtlOverride);
            Assert.Equal(expectedObject.ResolverContext["userid"], actualObject.ResolverContext["userid"]);
            Assert.Equal(expectedObject.ResolverContext["info"], actualObject.ResolverContext["info"]);
            Assert.Equal(expectedObject.ResolverContext["more_info"], actualObject.ResolverContext["more_info"]);
            Assert.Equal(expectedObject.DeniedFields.Count(), actualObject.DeniedFields.Count());
        }
    }
}
