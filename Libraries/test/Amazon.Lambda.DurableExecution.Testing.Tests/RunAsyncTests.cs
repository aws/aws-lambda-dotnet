...
#pragma warning restore CS0162
    ...
    {
        // Each sequential wait is a suspension point that costs one replay/invocation.
        // With time-skipping on, the waits fold to succeeded immediately (no real
        // clock delay), so the workflow legitimately re-invokes more times than
        // MaxInvocations allows and the limit fires quickly.
        await using var runner = new DurableTestRunner<string, string>(
            handler: async (input, ctx) =>
            {
                for (var i = 0; i < 10; i++)
                    await ctx.WaitAsync(TimeSpan.FromHours(1), name: $"wait-{i}");
                return "done";
            },
            options: new TestRunnerOptions { SkipTime = true, MaxInvocations = 3, DefaultTimeout = TimeSpan.FromSeconds(5) });
