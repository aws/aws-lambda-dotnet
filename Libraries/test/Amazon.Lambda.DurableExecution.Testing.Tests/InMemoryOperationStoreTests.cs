...
public class InMemoryOperationStoreTests
{
    ...
    [Fact]
    public void GetAllOperations_ReturnsSnapshot_NotLiveList()
    {
        var store = new InMemoryOperationStore();
        store.Upsert("arn:test", new Operation { Id = "op-1", Type = OperationTypes.Step });

        var snapshot = store.GetAllOperations("arn:test");
        // Mutating the store after the read must not change the returned snapshot.
        store.Upsert("arn:test", new Operation { Id = "op-2", Type = OperationTypes.Step });

        Assert.Single(snapshot);
        Assert.Equal(2, store.GetAllOperations("arn:test").Count);
    }
    ...
    public async Task ConcurrentUpserts_AreThreadSafe()
    {
        ...
        // Concurrent writers + readers must not corrupt the backing Dictionary/List
        // (regression coverage for the unsynchronized store).
        var tasks = Enumerable.Range(0, count).Select(i => Task.Run(() =>
        {
            store.Upsert("arn:test", new Operation { Id = $"op-{i}", Type = OperationTypes.Step });
            _ = store.GetAllOperations("arn:test");
            _ = store.OperationCount("arn:test");
        }));

        await Task.WhenAll(tasks);

        Assert.Equal(count, store.OperationCount("arn:test"));
        Assert.Equal(count, store.GetAllOperations("arn:test").Select(o => o.Id).Distinct().Count());
    }
}
