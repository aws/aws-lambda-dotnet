...
public class Function
{
    ...
    private async Task<string> Workflow(string input, IDurableContext context)
    {
        ...
        {
            var step1Result = await childContext.StepAsync(
                async (_, _ct) =>
                {
            ...
            var step2Result = await childContext.StepAsync(
                async (_, _ct) =>
                {
                    await Task.CompletedTask;
                    return step1Result;
            ...
            return step2Result;
        }, name: "multi-steps", config: new ChildContextConfig { SubType = "RunInChildContext" });
