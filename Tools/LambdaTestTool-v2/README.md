...
> The current `lambda.EmptyFunction` template targets `net10.0`, so building the generated project needs a .NET 10 SDK. If you only have .NET 8 or 9 installed, edit the `<TargetFramework>` in the generated `.csproj` to `net8.0` (or `net9.0`) before continuing.
...
| `--api-gateway-emulator-mode <MODE>` | API Gateway mode: `Rest`, `HttpV1`, or `HttpV2`. Required when the API Gateway emulator is started. | — |
...
| `--config-storage-path <PATH>` | Path for [saving settings and requests](#saving-requests). A relative path is resolved against the current directory. | — |
...
```
dotnet lambda-test-tool start --lambda-emulator-port 5050 --config-storage-path <path>
```
...
Set [`AWS_LAMBDA_RUNTIME_API`](#the-aws_lambda_runtime_api-environment-variable), then launch the function assembly under the test tool's runtime support shim. From the build output directory (e.g. `bin/Debug/net10.0`):
...
2. **`{TARGET_FRAMEWORK}`** — your Lambda project's target framework, e.g. `net10.0`.
